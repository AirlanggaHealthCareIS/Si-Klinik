...
public class TransaksiObat_Server extends UnicastRemoteObject implements TransaksiObat_Service {
    ...
    @Override
    public Transaksi_Obat getTO (String id_transaksi_obat) throws RemoteException{
        System.out.println("Client Melakukan Proses Get TO");

        PreparedStatement statement = null;
        
        try{
            statement = DatabaseUtilities.getConnection().prepareStatement("SELECT * FROM  `transaksi_obat` WHERE id_transaksi_obat = ?");
